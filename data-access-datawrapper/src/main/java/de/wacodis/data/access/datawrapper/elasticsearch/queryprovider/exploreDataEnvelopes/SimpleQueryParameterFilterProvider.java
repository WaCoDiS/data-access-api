    ...
    public QueryBuilder buildFilterForQueryParam(String fieldName, DataEnvelopeQueryQueryParams queryParam) {
        ...
        switch(queryParam.getComparator()){
            ...
            case LESSEROREQUALS:
                ...
                break;
            default:
                LOGGER.warn("unknown comparator {}, assume comparator {}, build TermQuery for field name {} to check for equality", queryParam.getComparator().toString(), DataEnvelopeQueryQueryParams.ComparatorEnum.EQUALS.toString(), fieldName);
                filter = new TermQueryBuilder(fieldName, queryParam.getValue());
                break;
        }
